...
public class TezSessionState {
  ...
  private TezClient startSessionAndContainers(TezClient session, HiveConf conf,
    ...
    try {
      ...
      if (conf.getBoolean("fact.keep.tez.queue.name", false)) {
        LOG.info("Keep tez.queue.name='{}' (id: {})", conf.get(TezConfiguration.TEZ_QUEUE_NAME), sessionId);
      } else {
        LOG.info("Clear tez.queue.name='{}' (id: {})", conf.get(TezConfiguration.TEZ_QUEUE_NAME), sessionId);
